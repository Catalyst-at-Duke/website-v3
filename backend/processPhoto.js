...
const FULLSIZE_IMAGE_LOCATION = path.resolve(os.tmpdir(), "images_temp");
...
// returns whether a file already exists in the bucket
const fileExists = async (filename) => {
  const file = bucket.file(`member_images/${filename}`);
  try {
    file.exists();
    return true;
  } catch (e) {
...
const downloadAndCompress = (properties, url) => {
  return new Promise(async (resolve, reject) => {
    ...
    writer.on("finish", () => {
      compress_images(
        filepath.replace(/\\/g, "/"),
        COMPRESSED_IMAGE_LOCATION.replace(/\\/g, "/"),
